...
let test_head = assert(hd stream_example == 0) (* początek to 0 *)

let test_tail = assert(hd (tl stream_example) == 1) (* ogon początku to wsm 1 *)
...
let test_map2 = assert(hd (tl stream_map2) == 2)
